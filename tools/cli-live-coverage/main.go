...
func main() {
	...
	runnable := collectRunnableCommands(cli.NewRootCommand())
	invoked, err := discoverLiveInvocations(*liveDir)
	if err != nil {
...
func discoverLiveInvocations(dir string) (invocations, error) {
	...
	err := filepath.WalkDir(dir, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		for _, declaration := range node.Decls {
			...
			commands := commandsInvokedBy(function)
			if len(commands) == 0 {
				continue
			}
...
func commandsInvokedBy(function *ast.FuncDecl) map[string]struct{} {
	...
	ast.Inspect(function.Body, func(n ast.Node) bool {
		...
		if path := commandPathFromArgs(call.Args); path != "" {
...
func commandPathFromArgs(args []ast.Expr) string {
	parts := []string{}
	...
	for index, arg := range args {
		...
		if strings.HasPrefix(value, "-") {
			if len(parts) == 0 {
				// A global flag such as --json or --dry-run, which precedes the
				// command itself.
				continue
			}
			// Once the command words have started, a flag ends them. Anything
			// after it is a flag or a flag value, not part of the path.
			break
		}
