...
func TestPRExtendedLifecycleAndReviewerCommands(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case request.Method == http.MethodGet && request.URL.Path == "/rest/api/latest/projects/TEST/repos/demo/pull-requests/30":
			writer.Header().Set("Content-Type", "application/json;charset=UTF-8")
			_, _ = writer.Write([]byte(`{"id":30,"title":"Feature PR","state":"OPEN","open":true,"closed":false,"version":1,"author":{"user":{"name":"author1"}}}`))
...
func TestApplyInferredRepositoryContext(t *testing.T) {
	...
	t.Run("annotated command skips inference", func(t *testing.T) {
		gitBackendFactory = func() git.Backend {
			return inferenceGitBackendStub{
				repoRoot: "/tmp/repo",
				remotes:  []git.Remote{{Name: "origin", URL: "https://bitbucket.local:7990/scm/PRJ/demo.git"}},
		...
		t.Setenv("BITBUCKET_PROJECT_KEY", "KEEP")
		t.Setenv("BITBUCKET_REPO_SLUG", "keep")

		cmd := &cobra.Command{Use: "serve", Annotations: map[string]string{annotationNoAmbientRepoInference: "true"}}
		cmd.Flags().String("repo", "", "")
		errBuffer := &bytes.Buffer{}
		cmd.SetErr(errBuffer)

		if err := applyInferredRepositoryContext(cmd, false); err != nil {
			t.Fatalf("expected annotated command to skip inference, got: %v", err)
		}

		if got := cmd.Flags().Lookup("repo").Value.String(); got != "" {
			t.Fatalf("expected repo flag to stay unset, got %q", got)
		}
		if got := os.Getenv("BITBUCKET_PROJECT_KEY"); got != "KEEP" {
			t.Fatalf("expected project key unchanged, got %q", got)
		}
		if got := os.Getenv("BITBUCKET_REPO_SLUG"); got != "keep" {
			t.Fatalf("expected repo slug unchanged, got %q", got)
...
func TestMCPServeOptsOutOfAmbientRepositoryInference(t *testing.T) {
	...
	if err := applyInferredRepositoryContext(serve, false); err != nil {
		t.Fatalf("apply inferred repository context failed: %v", err)
	}
