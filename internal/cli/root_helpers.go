...
// resolveReviewCounts measures a pull request's outstanding review feedback,
// preferring the activity timeline because it counts comment threads as well as
// tasks. Bitbucket 10.x drops the task counters from the single pull request
// payload, so when the timeline is unavailable this falls back to the
// blocker-comment count endpoint, which is one cheap request and still exact
// for tasks. If neither works the counts stay empty and the summary reports
// that nothing was measured rather than reporting zero.
...
// collectReviewSummaries resolves the review summary for each pull request. It
// runs one activity lookup per pull request sequentially, matching the rest of
// the codebase, which is why the caller exposes it behind an opt-in flag.
// The result is index-aligned with pullRequests.
...
// formatReviewStatusIndicator renders the per-row indicator used by
// `bb pr list --with-review-status`, where accurate thread counts are available.
...
// formatPullRequestCounts renders the compact per-row indicator used by
// `bb pr list`. It returns an empty string when the server reported nothing.
