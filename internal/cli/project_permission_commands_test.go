...
func TestCanonicalPermissionCommandsPointAtTheirAlias(t *testing.T) {
	cases := []struct {
		...
		{
			args:     []string{"repo", "settings", "security", "permissions", "users", "grant", "--help"},
			expected: "Also available as bb repo permissions grant, one level shallower.",
		},
		{
			args:     []string{"repo", "settings", "security", "permissions", "groups", "revoke", "--help"},
			expected: "Also available as bb repo permissions revoke --group, one level shallower.",
		},
		{
		...
		{
			args:     []string{"diff", "pr", "--help"},
			expected: "Also available as bb pr diff, which is the gh spelling.",
		},
	}
