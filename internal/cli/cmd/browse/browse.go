...
func New(deps Dependencies) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := d.URLOpener(targetURL); err != nil {
				return err
			}
...
func resolveBrowseRepositoryReference(selector string, cfg config.AppConfig) (repoRef, string, error) {
	trimmed := strings.TrimSpace(selector)
	if trimmed == "" {
		projectKey, slug, err := reposel.Resolve("", cfg)
		if err != nil {
			return repoRef{}, "", err
		}
		return repoRef{ProjectKey: projectKey, Slug: slug}, cfg.BitbucketURL, nil
	}

	host, ref, ok := parseHostQualifiedRepositorySelector(trimmed)
	if ok {
		hostURL := host
		if !strings.Contains(hostURL, "://") {
			hostURL = "https://" + hostURL
		}
		return ref, hostURL, nil
	}

	projectKey, slug, err := reposel.Parse(trimmed)
	if err != nil {
		return repoRef{}, "", err
	}
...
func parseHostQualifiedRepositorySelector(value string) (string, repoRef, bool) {
	...
	return host, repoRef{ProjectKey: project, Slug: slug}, true
}
