	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && strings.Contains(path, "/browse/"):
			...
			if r.URL.Query().Get("blame") == "true" {
				// blame is a list of spans, each naming an author and the run
				// of lines they last touched. The fixture used to make it an
				// object with one author, which is what the reader expected --
				// so the two agreed with each other and neither agreed with
				// Bitbucket, and the live suite was where that showed up.
				_, _ = w.Write([]byte(`{
					"blame": [
						{"author": {"name": "alice"}, "lineNumber": 1, "spannedLines": 1},
						{"author": {"name": "bob"}, "lineNumber": 2, "spannedLines": 1}
					],
...
func TestRepoBrowseBlame(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "alice") || !strings.Contains(buf.String(), "bob") {
		t.Fatalf("expected each line attributed to its own author, got: %s", buf.String())
	}
	...
	if !strings.Contains(buf.String(), `"author": "alice"`) || !strings.Contains(buf.String(), `"author": "bob"`) {
