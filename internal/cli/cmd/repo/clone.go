...
type cloneRepoRef struct {
	ProjectKey string
	Slug       string
}

func resolveRepositoryCloneInput(input string, cfg config.AppConfig) (cloneRepoRef, string, bool, error) {
	trimmed := strings.TrimSpace(input)
	if trimmed == "" {
		return cloneRepoRef{}, "", false, apperrors.New(apperrors.KindValidation, "repository is required", nil)
	}

	host, projectKey, slug, parsed := giturl.ParseBitbucketRemote(trimmed)
	if parsed {
		return cloneRepoRef{ProjectKey: projectKey, Slug: slug}, normalizeCloneHost(trimmed, host), true, nil
	}

	host, repoFromHostSelector, ok := parseHostQualifiedRepoRef(trimmed)
	if ok {
		hostURL := host
	...
	if err != nil {
		return cloneRepoRef{}, "", false, err
	}

	if strings.TrimSpace(cfg.BitbucketURL) == "" {
		if parsedSelector == cloneSelectorProjectSlug {
			return cloneRepoRef{}, "", false, apperrors.New(apperrors.KindValidation, "BITBUCKET_URL is required to clone repositories by PROJECT/slug", nil)
		}
		return cloneRepoRef{}, "", false, apperrors.New(apperrors.KindValidation, "BITBUCKET_URL is required to clone repositories by slug", nil)
	}
...
func parseCloneSelector(value, defaultProjectKey string) (cloneRepoRef, cloneSelectorKind, error) {
	trimmed := strings.TrimSpace(value)
	if trimmed == "" {
		return cloneRepoRef{}, cloneSelectorProjectSlug, apperrors.New(apperrors.KindValidation, "repository must be in PROJECT/slug format or a slug with BITBUCKET_PROJECT_KEY set", nil)
	}
	...
	if len(parts) == 1 {
		...
		if project == "" {
			return cloneRepoRef{}, cloneSelectorSlugOnly, apperrors.New(apperrors.KindValidation, "repository must be in PROJECT/slug format when BITBUCKET_PROJECT_KEY is not set", nil)
		}
		...
		return cloneRepoRef{ProjectKey: project, Slug: slug}, cloneSelectorSlugOnly, nil
	}

	if len(parts) != 2 {
		return cloneRepoRef{}, cloneSelectorProjectSlug, apperrors.New(apperrors.KindValidation, "repository must be in PROJECT/slug format", nil)
	}
	...
	return cloneRepoRef{ProjectKey: projectKey, Slug: slug}, cloneSelectorProjectSlug, nil
}

func parseHostQualifiedRepoRef(value string) (string, cloneRepoRef, bool) {
	trimmed := strings.Trim(strings.TrimSpace(value), "/")
	parts := strings.Split(trimmed, "/")
	if len(parts) != 3 {
		return "", cloneRepoRef{}, false
	}
	...
	return host, cloneRepoRef{ProjectKey: project, Slug: slug}, true
}
...
func resolveHTTPCloneURL(rawInput string, usedURLInput bool, cloneHost string, repo cloneRepoRef) (string, error) {
	if usedURLInput && isExplicitHTTPCloneURL(rawInput) {
		return strings.TrimSpace(rawInput), nil
	}
	return giturl.BuildBitbucketCloneURL(giturl.NormalizeHTTPCloneHost(cloneHost), repo.ProjectKey, repo.Slug)
}
...
func cloneRepositoryWithAuthFallback(
	...
	if hasStoredHTTPAuth && normalizeHTTPCloneBaseURL(httpCloneURL) != normalizeHTTPCloneBaseURL(resolvedHTTPCloneHost) {
		httpCloneURL, err = giturl.BuildBitbucketCloneURL(normalizeHTTPCloneBaseURL(resolvedHTTPCloneHost), repo.ProjectKey, repo.Slug)
		if err != nil {
			return "", err
		}
	}
...
func resolveSSHCloneURL(rawInput string, usedURLInput bool, cloneHost string, repo cloneRepoRef) (string, bool, error) {
	if usedURLInput {
...
func lookupParentCloneURL(ctx context.Context, cfg config.AppConfig, cloneHost string, repo cloneRepoRef) (string, string, error) {
	...
	parentURL, err := giturl.BuildBitbucketCloneURL(cloneHost, response.Origin.Project.Key, response.Origin.Slug)
	if err != nil {
		return "", "", err
	}
