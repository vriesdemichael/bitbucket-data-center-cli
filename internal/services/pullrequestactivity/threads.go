...
// computed over the unfiltered set so that it describes the pull request rather
// than whatever filter the caller happened to pass.
...
func (service *Service) TrySummarize(ctx context.Context, repository RepositoryRef, pullRequestID string, limit int) (*Summary, error) {
	activities, err := service.List(ctx, repository, pullRequestID, ListOptions{Limit: limit})
	if err != nil {
		if ctx.Err() != nil {
			return nil, err
		}
