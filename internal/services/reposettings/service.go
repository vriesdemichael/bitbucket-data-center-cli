...
func (service *Service) UpdateDefaultTask(ctx context.Context, repo RepositoryRef, taskId string, description string, sourceRef *string, targetRef *string) (*DefaultTask, error) {
	...
	//
	// The effect was silent and one-directional. `default-task update
	// --description` on a task scoped to feature/* -> main reset both matchers
...
func (service *Service) findDefaultTask(ctx context.Context, repo RepositoryRef, taskID string) (DefaultTask, error) {
	...
	return DefaultTask{}, apperrors.New(apperrors.KindNotFound,
		"default task "+taskID+" does not exist on "+repo.ProjectKey+"/"+repo.Slug, nil)
}
