...
2. Native CODEOWNERS Evaluation by Default: When creating a pull request, `bb pr create` inspects the
   diff against `.bitbucket/CODEOWNERS` (or `CODEOWNERS`) from the target branch or local workspace,
   resolving matching code owners (`--codeowners` defaults to true). If the repository does not use
   CODEOWNERS, it is silently skipped without error. Users can opt out via `--no-codeowners`.
...
3. Fine-Grained Group Selection Strategies: Group references in `.bitbucket/CODEOWNERS` support
   ...
   Reviewer group names containing spaces are supported via backslash escaping (`@backend\\ engineers:random(2)`).
...
4. Attaching Automated Reviewers to Open Pull Requests: `bb pr review reviewer add <id>` provides
   ...
   an existing pull request after creation.
