...
- `Keep --token and --password behind a stderr warning`: The v3 behaviour. The warning moved the default without moving the exposure: the flags kept being copied into CI jobs and IDE configs, where the value sits in the process argument list for as long as the process runs. A break of this kind belongs in a major, and v4 is one.
