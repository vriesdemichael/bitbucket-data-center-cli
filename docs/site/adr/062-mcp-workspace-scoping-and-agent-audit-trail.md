...
bb ai mcp serve accepts --project and --repo, confining every tool call to that project or repository, and --audit-file, recording every invocation as JSON Lines. Both are off by default. Both are enforced in one middleware over tools/call, never in handlers. Each tool has a scope rule, held to the catalogue by a test. A tool whose arguments identify its target has them injected when omitted and refused when they name something else. A tool whose project argument is only a filter, or that addresses something Bitbucket does not scope to a project at all, is withheld: dropped from the catalogue as well as refused on call. list_pull_requests is the one tool whose project and repository are both optional, because leaving the repository out asks something of its own: the pull requests of the caller, across repositories. Under a project scope a call without a repository answers with those pull requests narrowed to the project. That filters what the dashboard answered, and is accepted because the dashboard holds only the work of the caller and the filter decides everything the agent receives. Audit writes are synchronous and a failed write refuses the call, unless --audit-failure=warn. The sink is a file or stderr; stdout carries the protocol and is rejected. Administrators can mandate the destination with policy.mcp_audit_file, which binds only where the policy file is out of the developer's reach -- see ADR-058 for the deployment step that makes that true.
...
- `Instead of narrowing list_pull_requests under a project scope, refuse a call without a repository`: Leaves an agent confined to a project no way to ask for its own pull requests, and refuses an omitted argument the scope is meant to fill in.
