...
Every `bb ...` invocation inside a shell-tagged fenced block in the repository's Markdown is parsed against the real Cobra command tree by tools/docs-lint, which runs in quality:verify, in the pre-push hook and in CI. A documented command that does not resolve, uses a flag that does not exist, or passes the wrong number of positional arguments fails the build. Validation is static: the linter resolves the command, parses its flags and applies its argument rules without executing anything and without contacting a Bitbucket instance. Blocks that deliberately show a malformed invocation are marked with an HTML comment, `<!-- docs-lint: expect-invalid -->`, which inverts the check rather than skipping it. Such a block fails if its command becomes valid. A json or yaml block is in scope for two further checks. An invocation a client launches from configuration -- `"command": "bb"` beside an `"args"` array, as every MCP client spells it -- is reconstructed and validated by the same rules a shell line gets. A block showing machine output is checked against the schema its command declares, read from the same lookup --describe answers with, so an example and the flag cannot disagree. The block says which command it came from with `<!-- docs-lint: output-of bb <command> -->`, or declares itself an illustration of the envelope rather than one command's payload with `<!-- docs-lint: envelope-shape -->`; an illustration is still held to the envelope rules. A machine-output block carrying neither directive fails, so a new example is checked by default rather than when somebody remembers. Text blocks remain out of scope, which is what excludes the generated command reference, whose `bb ...` lines are Cobra usage strings and are correct by construction.

## Agent Instructions

Run `task docs:lint` after changing documentation or the command tree. It is part of `task quality:verify`, so the pre-push hook covers it. Write examples in ```bash blocks so they are checked. A snippet placed in an unmarked or text block is invisible to the linter, which is a way of avoiding the check rather than passing it. When an example is meant to be invalid, mark the block with `<!-- docs-lint: expect-invalid -->` rather than reformatting it to escape the check. Do not add an unconditional ignore directive. When you add a machine-output example, mark it with `<!-- docs-lint: output-of bb <command> -->` so it is checked against that command's schema, or `<!-- docs-lint: envelope-shape -->` when it illustrates the envelope rather than one command's payload. Do not reach for envelope-shape to quiet a schema failure: the failure means the example and the command disagree, and the command is right. Do not paraphrase a payload from memory. Read it from the command's --describe output, which is the same declaration this check reads. When the linter reports something you believe is correct documentation, suspect the linter and check for a trailing carriage return first: a CRLF checkout puts \r inside the last token, and pflag reports that as an unknown flag with no visible difference in the message.
...
- `Infer which command an output example belongs to from the nearest preceding invocation`: A shell block commonly lists several commands, and the output shown belongs to one of them rather than to the last. Inference read a bb auth server use payload as bb auth status output, which is how its wrong status field went unreported. A declared binding also tells a human reader which command produced the block.
