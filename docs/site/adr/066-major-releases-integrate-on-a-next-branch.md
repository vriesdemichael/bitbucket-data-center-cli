...
1. Where work goes:
   ...
   - Whether a change is breaking no longer decides its branch -- everything goes to `next` --
     ...
     invocation which previously succeeded, is breaking even when it is a bug fix. Several of the
     Phase 0 fixes were exactly that: rejecting an out-of-range --expiry-days is a fix, and it
   ...
   - Restoring behaviour that an accepted ADR or a shipped release note already specified is not
     ...
     it. Marking such a fix breaking cuts a major for finishing work already announced. #570 and
     #569 are both that shape: ADR-073 requires --yes on destructive commands and the v4 notes
     promise exit 2 for invalid arguments, and the binary kept neither.
