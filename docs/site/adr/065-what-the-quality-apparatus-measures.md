...
1. Coverage axes, and the distinct question each answers:
   ...
   - Patch line coverage (at least 85% on patch sets of 30 or more coverable lines; at most 2
     uncovered changed lines below that) blocks new untested code. It is the axis that fires most
     often and it earns its cost: it caught unreachable defensive branches that would otherwise
     have shipped as code no test could exercise.
   - Global line coverage (at least 85% combined, scoped to cmd/ and internal/ excluding generated
...
Line coverage keeps its 85% but not for the reason usually given. The patch gate demonstrably catches new untested code. The global floor sits at 87.41% against an 85% floor, so 2.41 points of headroom, and ADR-049 measured that adding tools/ would cut that to 1.08. It is therefore close enough to bite on an unrelated change, which is tolerable only because its job is narrow and now stated: erosion, not new code. Thin headroom is a signal to add tests, never to lower the floor.
