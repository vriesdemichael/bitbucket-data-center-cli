...
- **Structured Error Taxonomy ([ADR-011](../adr/011-error-taxonomy-and-cli-exit-contract.md), [ADR-046](../adr/046-json-error-envelope-on-the-failure-path.md))**: Fatal failures emit a predictable JSON error envelope with categorized error taxonomy (`validation`, `authentication`, `authorization`, `not_found`, `conflict`, `transient`, `permanent`, `cancelled`, `unknown_outcome`, `not_implemented`, `internal`).
