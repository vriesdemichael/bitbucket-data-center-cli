...
Global Flags:
      ...
      --describe                 Say what this command's --json output looks like, for a run and for --dry-run, instead of running it
      --dry-run                  Say what the real run would do, without doing it; each command's help says what it checks
...
Additional help topics:
  bb dry-run        What --dry-run answers, and how far to trust it
...
```text
Write an agent skill file (defaults to "bb") where coding agents read
...
Dry run:
  Shows that it would write the skill file, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Shows that it would delete the skill file, without doing it; nothing is checked first (predicted)
```
...
```text
Print an agent skill to stdout (defaults to "bb").
...
Dry run:
  Shows that it would add host aliases, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Shows that it would replace host aliases, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Shows that it would remove a host alias, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Shows the change it would make, without making it; nothing is checked first (predicted)
```
...
Dry run:
  Shows that it would store credentials, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Shows that it would remove stored credentials, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Shows that it would change the default host, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Shows that it would configure git to authenticate through bb, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Shows that it would clone into a new directory, without doing it; nothing is checked first (predicted)
```
...
```text
--dry-run asks what the real run would do, without doing it. Each command's
...
Dry run:
  Shows that it would create a local branch, without doing it; nothing is checked first (predicted)
```
...
Dry run:
  Reports the release it would install and whether it verifies, without installing it
```
