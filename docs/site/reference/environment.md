...
| `BB_CONFIG_PATH` | `%AppData%\bb\config.yaml`, `~/Library/Application Support/bb/config.yaml` or `~/.config/bb/config.yaml` | Path to the stored configuration file. See [Two identities on one host](../advanced/repository-discovery-and-server-switching.md#two-identities-on-one-host). |
